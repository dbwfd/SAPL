...
class cross_filter(nn.Module):
    def __init__(self,config,text_size,visual_size,downsample_rate,model_size,nhead,feedforward_size,dropout,
        ...
        self.is_t2v=is_t2v
        self.is_relu=config.IS_RELU
        if self.is_t2v:
            if self.is_relu == True:
                self.text2visual=nn.Sequential(OrderedDict([
                    ("linear1", nn.Linear(text_size, (text_size+ visual_size)// 2)),
                    ("relu", nn.ReLU(inplace=True)),
                    ("linear2", nn.Linear((text_size+ visual_size)// 2, visual_size))
                ])) 
            else:
                self.text2visual=nn.Sequential(OrderedDict([
                    ("linear1", nn.Linear(text_size, (text_size+ visual_size)// 2)),
    ...
    def forward(self,visual_embd,text_embd):
        ...
        out=self.filterlayer(text_embd2,visual_embd) #b 1 c
...
class SCTransNet(nn.Module):
    def __init__(self, config, n_channels=1, n_classes=1, img_size=256, vis=False, mode='train', deepsuper=True):
        ...
        if self.is_wbfilter:
            ...
            self.cafilter5=cross_filter(config,self.text_size,in_channels*8,2,in_channels*8,1,in_channels*8,0,SAtt=self.is_satt,feedforward=self.is_feedforward,
                                       filter_type=config.Filter_type,is_t2v=True)
            
            self.cafilter4=cross_filter(config,self.text_size,in_channels*4,4,in_channels*4,1,in_channels*4,0,SAtt=self.is_satt,feedforward=self.is_feedforward,
                                       filter_type=config.Filter_type,is_t2v=True)
            self.cafilter3=cross_filter(config,self.text_size,in_channels*2,8,in_channels*2,1,in_channels*2,0,SAtt=self.is_satt,feedforward=self.is_feedforward,
                                       filter_type=config.Filter_type,is_t2v=True)

            self.cafilter2=cross_filter(config,self.text_size,in_channels,16,in_channels,1,in_channels,0,SAtt=self.is_satt,feedforward=self.is_feedforward,
                                       filter_type=config.Filter_type,is_t2v=True)
            
            self.cafilter1=cross_filter(config,self.text_size,in_channels,16,in_channels,1,in_channels,0,SAtt=self.is_satt,feedforward=self.is_feedforward,
                                       filter_type=config.Filter_type,is_t2v=True)
    ...
    def forward(self, x, feature=None, is_train=True):
        ...
        if self.is_prompt_learner:
            if feature is not None and self.config.IS_VISUAL:
                text_embd = self.custom_clip(feature)
            else:
