...
parser.add_argument("--epochs", default=500, type=int, help="optimizer name: AdamW, Adam, Adagrad, SGD")
...
parser.add_argument("--split", default="sc", type=str, help="dataset split")
...
def train(run):
    ...
    train_set = TrainSetLoader(dataset_dir=opt.dataset_dir, dataset_name=opt.dataset_name, patch_size=opt.patchSize,
                               img_norm_cfg=opt.img_norm_cfg,model=opt.model_clip,split=opt.split)
    train_loader = DataLoader(dataset=train_set, num_workers=opt.threads, batch_size=opt.batchSize, shuffle=True)
    test_set = TestSetLoader(opt.dataset_dir, opt.dataset_name, opt.dataset_name, img_norm_cfg=opt.img_norm_cfg,model=opt.model_clip,split=opt.split)
    test_loader = DataLoader(dataset=test_set, num_workers=8, batch_size=
                             16, shuffle=False)
...
sweep_configuration = {
    ...
    "parameters": {
        ...
        "TOKEN_POSITION": {"values": [ "middle","end"]}, # promp position
        ...
        "is_feedforward": {"values": [False, True]},
        "drop_out": {"values": [0.0]},
        "is_filter": {"values": [True]},
          # filter type
    },
}
...
def main():
    ...
    with wandb.init(project="text") as run:
        ...
        config_vit.IS_FILTER=run.config.is_filter
        print("Current config_vit:",config_vit)
        log_dir = f"./sweep_logs_iv14/{config_vit.Filter_type}_POS{config_vit.TOKEN_POSITION}_NCTX{config_vit.N_CTX}_FF{config_vit.transformer.IS_FEEDFORWARD}_DP{config_vit.Filter_DROPOUT}"
        opt.save = log_dir
