...
class TrainSetLoader(Dataset):
    def __init__(self, dataset_dir, dataset_name, patch_size, img_norm_cfg=None,model="ViT-B/16",split="sc"):
        ...
        assert model in ["ViT-B/32","ViT-B/16","ViT-L/14"],"wrong model name !!!"
           
        if model=="ViT-B/32":
            self.model_dir="B32/"
            print("Using B32 features")
        elif model=="ViT-B/16":
            self.model_dir="B16/"
            print("Using B16 features")
        elif model=="ViT-L/14":
            self.model_dir="L14/"
            print("Using L14 features")
    ...
    def __getitem__(self, idx):
        try:
            ...
            feature = np.load((self.dataset_dir + '/features/' + self.model_dir + self.train_list[idx] + '.npy').replace('//', '/'))
        except:
        ...
        feature = torch.from_numpy(np.ascontiguousarray(feature))  # numpy 转tensor
        return img_patch, mask_patch, feature.to(torch.float32)
...
class TestSetLoader(Dataset):
    def __init__(self, dataset_dir, train_dataset_name, test_dataset_name, img_norm_cfg=None,model="ViT-B/16",split="sc"):
    ...
    def __getitem__(self, idx):
        try:
            ...
            feature = np.load((self.dataset_dir + '/features/' + self.model_dir + self.test_list[idx] + '.npy').replace('//', '/'))
        except:
        ...
        feature = torch.from_numpy(np.ascontiguousarray(feature))
        if img.size() != mask.size():
            print('111')
        return img, mask, feature.to(torch.float32), [h, w], self.test_list[idx]
