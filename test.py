...
def test(test_net=None,device=None,dataset_name=None,dataset=None):
    if device is not None:
        testopt.device = device
    if dataset_name is not None:
        testopt.test_dataset_name = dataset_name
        testopt.train_dataset_name = dataset_name
    ...
    if dataset is not None:
        ...
        test_set = TestSetLoader(testopt.dataset_dir, testopt.train_dataset_name, testopt.test_dataset_name, testopt.img_norm_cfg)
    ...
    with torch.no_grad():
        for idx_iter, (img, gt_mask, feature,size, img_dir) in enumerate(tbar):
            # img = Variable(img)
            ...
            # CUDA:
            pred = net.forward(img.to(testopt.device), feature.to(testopt.device), False)
            pred = pred[:, :, :size[0], :size[1]]
            ...
            eval_05.update((pred[0, 0, :, :] > testopt.threshold).cpu(), gt_mask[0, 0, :, :], size)  # 目标
            ROC_05.update(pred, gt_mask)
            # save img
            if testopt.save_img == True:
                img_save = transforms.ToPILImage()((pred[0, 0, :, :]).cpu())
                if not os.path.exists(testopt.save_img_dir + testopt.test_dataset_name + '/' + testopt.model_name):
                    os.makedirs(testopt.save_img_dir + testopt.test_dataset_name + '/' + testopt.model_name)
                img_save.save(testopt.save_img_dir + testopt.test_dataset_name + '/' + testopt.model_name + '/' + img_dir[0] + '.png')
        ...
        return mIOU*100,nIoU*100,results2[0]*100,results2[1]*1e+6,F1_score*100


def test_main():
    testopt.f = open(testopt.save_log + 'test_' + (time.ctime()).replace(' ', '_').replace(':', '_') + '.txt', 'w')
    if testopt.pth_dirs == None:
        for i in range(len(testopt.model_names)):
            testopt.model_name = testopt.model_names[i]
            print(testopt.model_name)
            testopt.f.write(testopt.model_name + '_400.pth.tar' + '\n')
            for dataset_name in testopt.dataset_names:
                testopt.dataset_name = dataset_name
                testopt.train_dataset_name = testopt.dataset_name
                testopt.test_dataset_name = testopt.dataset_name
                print(dataset_name)
                testopt.f.write(testopt.dataset_name + '\n')
                testopt.pth_dir = testopt.save_log + testopt.dataset_name + '/' + testopt.model_name + '_400.pth.tar'
                test()
            print('\n')
            testopt.f.write('\n')
        testopt.f.close()
    else:
        for model_name in testopt.model_names:
            for dataset_name in testopt.dataset_names:
                for pth_dir in testopt.pth_dirs:
                    # if dataset_name in pth_dir and model_name in pth_dir:
                    testopt.test_dataset_name = dataset_name
                    testopt.model_name = model_name
                    testopt.train_dataset_name = pth_dir.split('/')[0]
                    print(pth_dir)
                    testopt.f.write(pth_dir)
                    print(testopt.test_dataset_name)
                    testopt.f.write(testopt.test_dataset_name + '\n')
                    testopt.pth_dir = testopt.save_log + pth_dir
                    test()
                    print('\n')
                    testopt.f.write('\n')
        testopt.f.close()
