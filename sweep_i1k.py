...
parser.add_argument("--epochs", default=650, type=int, help="optimizer name: AdamW, Adam, Adagrad, SGD")
parser.add_argument("--begin_test", default=0, type=int)
...
def train(run):
    ...
    for idx_epoch in tqdm(range(epoch_state, opt.nEpochs),desc="Epochs"):
        ...
        if (idx_epoch + 1) >= opt.begin_test and (idx_epoch + 1) % opt.every_test == 0:
            ...
            miou=eval_iou(model=net, dataloader=test_loader, device=opt.device, threshold=opt.threshold)
...
def main():
    with wandb.init(project="text") as run:
        ...
        if opt.model_clip=="ViT-B/32":
            model_dir="B32"
            
        elif opt.model_clip=="ViT-B/16":
            model_dir="B16"
            
        elif opt.model_clip=="ViT-L/14":
            model_dir="L14"
        print("Current config_vit:",config_vit)
        log_dir = f"./sweep_logs/{model_dir}/{config_vit.Filter_type}_POS{config_vit.TOKEN_POSITION}_NCTX{config_vit.N_CTX}_CLASS{config_vit.CLASS_NAME[0].replace(' ','_')}"
        opt.save = log_dir
